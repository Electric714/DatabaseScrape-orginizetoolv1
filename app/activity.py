...
def emit(level, message, *, job_id=None, source_id=None, **details):
    install()
    logging.getLogger("app.activity").log(
...
async def bundle():
    ...
    summary = {
        "application": "Paralegal Database Tool",
        "version": APP_VERSION,
        ...
        "recent_jobs": [
            {k: v for k, v in job.items() if k not in {"source_name", "message"}}
            for job in jobs
        ],
    ...
    def create_zip():
        ...
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as archive:
            ...
            archive.writestr(
                "activity.json",
                json.dumps(events["items"], indent=2, ensure_ascii=False),
            )
            archive.writestr(
                "activity.txt",
                "\n".join(
