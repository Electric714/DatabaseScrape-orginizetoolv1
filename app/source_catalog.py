...
SOURCE_CATALOG = [
    ...
    {"key": "bbb", "name": "Better Business Bureau", "method": "Published profile sitemaps + local profile navigation", "status": "Sitemap discovery verified; resilient local profile collection",
     "fields": ["better_business_bureau_complaints"], "hosts": ["bbb.org", "www.bbb.org"],
     "note": "BBB's published business-profile sitemap index is used and /search is never requested. POC sitemap ranges cover FL, IL, MN, MO, OH, and WI. Discovered profiles use narrowly scoped local Chromium navigation. A blocked, throttled, challenged, or unparsable profile remains unknown only for the affected contractor while unrelated contractors continue; exact profile identity/location is required before /complaints can propose a value."},
