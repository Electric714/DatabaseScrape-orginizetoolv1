...
SOURCE_CATALOG = [
    ...
    {"key": "bbb", "name": "Better Business Bureau", "method": "Targeted HTML", "status": "Blocked before parsing",
     "fields": ["better_business_bureau_complaints"], "hosts": ["bbb.org", "www.bbb.org"],
     "note": "Live Windows diagnostics hit an access challenge on /search before any profile was parsed. BBB's current robots policy also disallows generic query-string routes; no supported automated search/API acquisition path has been validated. No result is treated as zero complaints."},
