...
# Captured from the Public V2 data dictionary.  Spelling aliases below accommodate
# the CSV label changes observed between published revisions without guessing data.
...
FIELD_ALIASES = {
    ...
    "aliases": ("Aliases", "Alias"), "uei": ("UEI", "UEI SAM", "Unique Entity ID"),
    "cage_code": ("CAGE Code", "CAGE"), "address_1": ("Address 1", "Address Line 1"),
    "address_2": ("Address 2", "Address Line 2"), "city": ("City",),
    "state": ("State", "State / Province", "State Or Province Code"),
    "zip": ("ZIP", "ZIP Code", "Postal Code"), "country": ("Country", "Country Code"),
    "exclusion_type": ("Exclusion Type",), "exclusion_program": ("Exclusion Program",),
...
def iter_public_v2_rows(path: str | Path, *, max_uncompressed: int, artifact: dict | None = None) -> Iterator[dict]:
    """Yield normalized rows without materializing the extract."""
    path = Path(path)
    try:
    ...
    with archive:
        ...
        try:
            ...
            if missing:
                raise SamExtractError("SAM CSV schema drift; missing required columns: " + ", ".join(missing))
            count = 0
            for line, values in enumerate(reader, 2):
                ...
                if status in ACTIVE_VALUES:
                    ...
                    raise SamExtractError(f"unrecognized SAM active-status representation at row {line}")
