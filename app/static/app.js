...
async function loadJobs() {
  ...
  $('jobs').innerHTML = jobs.length ? jobs.map(job => `<article class="job"><div class="job-head"><strong>${esc(job.source_name || 'Research source')}</strong><span class="tag ${esc(String(job.status || '').toLowerCase())}">${esc(job.status || 'unknown')}</span></div><span class="job-time">${esc(humanDate(job.started_at))}</span><p class="job-message">${esc(job.message || 'No status message supplied.')}</p><p class="job-acquisition"><b>Acquisition:</b> ${esc(job.acquisition_status || 'pending')} · ${esc(job.acquisition_stage || 'prepare')}${job.failure_code ? ` · ${esc(job.failure_code)}` : ''}${job.retry_class && job.retry_class !== 'never' ? ` · retry: ${esc(job.retry_class.replaceAll('_',' '))}` : ''}</p><div class="job-stats"><span><b>${job.pages_processed ?? 0}/${job.pages_discovered ?? 0}</b> pages</span><span><b>${job.records_found ?? 0}</b> found</span><span><b>${job.records_new ?? 0}</b> new</span><span><b>${job.records_updated ?? 0}</b> updated</span><span><b>${job.errors ?? 0}</b> errors</span></div></article>`).join('') : '<div class="empty-state">No research jobs have run yet.</div>';
